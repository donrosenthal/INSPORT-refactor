...
class PDFProcessingService:
    ...
    def __init__(self, base_temp_dir=None):
        ...
        Args:
            base_temp_dir (str, optional): Base directory for temporary processing files.
    ...
    def _job_directory(self):
        ...
        Yields:
            str: Path to the newly created job directory.
        """
    ...
    def process_document(self, pdf_path, output_file=None, languages=["eng"]):
        ...
        This method determines whether a PDF is scanned or digital,
        and then extracts the text accordingly. It also handles PDFs
        with annotations.

        Args:
            pdf_path (str): Path to the input PDF file.
            output_file (str, optional): Path where extracted text should be saved.
                                         If None, a path will be generated within the job directory.
                                         Defaults to None.
            languages (list, optional): List of language codes for OCR.
                                        Defaults to ["eng"].
        ...
        Returns:
            dict: Processing result with keys:
                ...
                - document_type (str): Type of document detected (scanned, digital, digital_with_annotations).
    ...
    def _is_scanned_pdf(self, pdf_path, max_pages=5, min_chars_per_page=50):
        ...
        This method checks a few pages of the PDF to determine if they
        contain a minimal amount of text, which is indicative of a scanned document.

        Args:
            pdf_path (str): Path to the PDF file.
            max_pages (int, optional): Maximum number of pages to check. Defaults to 5.
            min_chars_per_page (int, optional): Minimum characters per page to consider as text.
                                                Defaults to 50.
        ...
        print(
            f"Scanned pages detected: {empty_pages}/{pages_checked} ({scanned_percentage:.0%})"
        )
    ...
    def _process_scanned_pdf(self, pdf_path, output_file, job_dir, languages=["eng"]):
        ...
        This method converts each page of the PDF to an image, then uses
        Tesseract OCR to extract the text from each image.
        ...
        """
        print(
            f"📄 PDF appears to be scanned. Running OCR with languages: {', '.join(languages)}..."
        )
    ...
    def _process_digital_pdf(self, pdf_path, output_file, has_annotations=False):
        ...
        This method extracts text from a digital PDF, preserving structure.
        If the PDF has annotations, it extracts and appends them to the text.

        Args:
            pdf_path (str): Path to the PDF file.
            output_file (str): Path where extracted text should be saved.
            has_annotations (bool, optional): Whether the PDF has annotations.
                                              Defaults to False.
    ...
    def _extract_structured_text(self, pdf_path):
        ...
        This method extracts text blocks from the PDF, which somewhat
        preserves the paragraph structure of the original document.
    ...
    def _extract_annotations(self, pdf_path):
        ...
        This method extracts various types of annotations, such as highlights
        and comments, from the PDF.

        Args:
            pdf_path (str): Path to the PDF file.

        Returns:
            list: List of annotation dictionaries. Each dictionary contains:
                - page (int): Page number of the annotation.
                - type (str): Type of annotation (e.g., "Highlight").
                - content (str): Comment associated with the annotation.
                - rect (list): Coordinates of the annotation rectangle.
                - highlighted_text (str): Text covered by the annotation (for highlights).
        """
        doc = fitz.open(pdf_path)
        total_pages = len(doc)
        annotations = []
        ...
        for page_num in tqdm(
            range(total_pages), desc="Extracting annotations", unit="page"
        ):
            page = doc[page_num]
    ...
    def check_dependencies(self):
        ...
        This method checks for the presence of PyMuPDF, tqdm, and Tesseract OCR.
    ...
    def cleanup_base_dir(self):
        ...
        This method is registered with atexit to ensure cleanup on exit.
        """
    ...
    def get_pdf_info(self, pdf_path):
        ...
        Returns:
            dict: PDF information, including:
                - page_count (int): Number of pages in the PDF.
                - metadata (dict): PDF metadata.
                - is_encrypted (bool): Whether the PDF is encrypted.
                - permissions (int): PDF permissions.
